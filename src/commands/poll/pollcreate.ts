  ...
  async execute(message: Message, args: string[]): Promise<void> {
    try {
      ...
      setTimeout(async () => {
        ...
        pollVoteObj?.sort((a, b) =>
          (a.voteCount as number) < (b.voteCount as number)
            ? 1
        ...
        if (result?.votes != undefined)
          ...
          let pollMessage2 = await (channel as TextChannel).messages.fetch(result?.messageID as string)
      ...
      this.messages.success(
        "Poll Created",
        `Your poll has been sent to <#${channel?.id}> and has the id \`#${pollId}\``,
        message
      );

      this.client.polldb.addPoll(
        message.member as GuildMember,
        title,
        description,
        options,
        (channel as TextChannel).id,
        pollMessage?.id as string,
        startdate.toString(),
        enddate.toString(),
        msDurration,
        pollId,
