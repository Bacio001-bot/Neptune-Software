...
class EmbedCommand extends Command_1.default {
    ...
    async execute(message, args) {
        try {
            ...
            if (channel) {
                let fetchedMessage = await (channel === null || channel === void 0 ? void 0 : channel.messages.fetch(args[1]).catch(console.error));
                if (fetchedMessage) {
                    ...
                    return this.messages.success("Edit Embed Success", `Message with the id \`${fetchedMessage.id}\` has been edited`, message);
                }
                else {
                    return this.messages.error("Edit Embed Error", `Not a valid messageID supplied`, message);
                }
            }
            else {
                return this.messages.error("Edit Embed Error", `Not a valid channel supplied`, message);
            }
        }
        catch (err) {
            console.log(err);
            return this.messages.error("Edit Embed Error", `A error occured please contact the developer`, message);
        }
    }
}
